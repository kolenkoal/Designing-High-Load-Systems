...
|------|-----------|-----------|----------|---------|----------|
| `app-pool` | Сервисы приложения + workers | **15** | 16 | 32 ГБ | 100 ГБ SSD |
...
**Итого: 23 ноды Kubernetes.**

Обоснование размера `app-pool`: суммарный CPU limit всех контейнеров (таблица 11.2.2) — 217 vCPU. При профиле 16 vCPU на ноду: ⌈217/16⌉ = 14 рабочих нод + 1 резервная = **15 нод**.

### 11.2.2 Контейнеры приложения (`app-pool`)

Пиковый RPS из раздела 2.2.3. Производительность FastAPI (async I/O) — ≈ 500 RPS/vCPU, API Gateway — ≈ 1 000 RPS/vCPU.

**CPU request** отражает среднее потребление (пик ÷ 3). Для сервисов на критическом пути (API Gateway, Auth Service) учитывается **фактор амплификации**: каждый входящий запрос порождает несколько внутренних вызовов.

- **API Gateway**: на каждый входящий запрос — вызов Auth Service + проксирование в downstream. Фактор амплификации ×3. Эффективная нагрузка: 20 895 × 3 = 63 000 RPS. CPU limit/реплику: 63 000 ÷ 1 000 ÷ 11 ≈ 6 000m. CPU request = limit ÷ 3 = 2 000m.
- **Auth Service**: вызывается на каждый входящий запрос (20 895 RPS) + lookup в Redis или PostgreSQL. Фактор амплификации ×2. Эффективная нагрузка: 20 895 × 2 = 41 790 RPS. CPU limit/реплику: 41 790 ÷ 500 ÷ 4 ≈ 21 000m. CPU request = limit ÷ 3 ≈ 7 000m.

| Сервис | Реплики | CPU request | CPU limit | RAM request | RAM limit | Обоснование реплик |
|--------|---------|------------|----------|------------|----------|-------------------|
| `api-gateway` | **11** | 2000m | 6000m | 512 Mi | 2 Gi | 20 895 × 3 амплификация ÷ 1 000 RPS/vCPU ÷ 11 реплик |
| `auth-service` | **4** | 7000m | 21000m | 1 Gi | 4 Gi | 20 895 × 2 амплификация ÷ 500 RPS/vCPU ÷ 4 реплики |
...
**Суммарно (лимиты):** 11×6 + 4×21 + 14×1 + 13×2 + 3×1 + 3×1 + 2×1 + 5×2 + 3×1 + 3×2 = 66 + 84 + 14 + 26 + 3 + 3 + 2 + 10 + 3 + 6 = **217 vCPU**, ~60 ГБ RAM limit.
...
| Kubernetes `app-pool` | 15 | 16 | 32 ГБ | 100 ГБ SSD | 240 | 480 ГБ |
