# Сервис краткосрочной аренды жилья — проектирование нагрузки
...
|-------------|---------------|-----------|----------------|-------------|
| Поиск | 200 млн | 6945 | Часть (cache miss) | Основной объём отдаёт CDN/поисковый кэш |
| Страницы объявлений | 175 млн | 6075 | Часть (cache miss) | Метаданные и API — ДЦ, медиа — CDN |
| Проверка доступности дат | 175 млн | 6075 | Полностью или через кэш | Критично к актуальности, при кэше — TTL короткий |
| Бронирование | 1.34 млн | 47 | 100% | Только primary ДЦ |
...
Оценка RPS в ДЦ при 30% cache miss для поиска и страниц объявлений: (6945 + 6075) × 0.3 + 6075 + 47 + 2 + 17 + 1734 ≈ 3906 + 6075 + 1800 ≈ **~11 800 RPS пик** в один ДЦ (остальное забирают CDN и кэш).

## 3.7 Схема DNS балансировки (при наличии)

Для одного ДЦ классическая DNS-балансировка между несколькими ДЦ не используется: одна запись A/AAAA указывает на один ДЦ (или на балансировщик перед ним).

**Вариант для этапа с двумя ДЦ (на будущее):**

- Одна доменная зона, один hostname (например, `api.example.com`).
- DNS возвращает несколько A-записей (IP первого и второго ДЦ); клиент или резолвер выбирает первый ответ — равномерного распределения нет, возможен GeoDNS.
- **GeoDNS:** поддомены или одна зона с географической маршрутизацией: например, пользователи из Americas → ДЦ в США, остальные → ДЦ в Европе. TTL 60–300 с для возможности переключения при сбоях.

Для MVP достаточно одной A-записи на основной ДЦ.

## 3.8 Схема Anycast балансировки (при наличии)

**Anycast** — один IP-адрес объявляется из нескольких точек присутствия (PoP); BGP направляет клиента в «ближайший» по маршруту узел.

- **При одном ДЦ:** Anycast не даёт выигрыша — одна точка объявления IP.
- **При нескольких ДЦ:** один Anycast-IP для API/веб-трафика возможен: трафик автоматически идёт в ближайший ДЦ по BGP. Минусы: нет явного контроля долей трафика между ДЦ, переключение при сбое зависит от BGP (сходимость 1–5 минут).

Для MVP Anycast не применяется. При масштабировании на несколько ДЦ Anycast можно использовать для «ближайшего» входа, комбинируя с DNS для более тонкого управления.

## 3.9 Механизм регулировки трафика между ДЦ (при наличии)

При одном ДЦ регулировка трафика между ДЦ отсутствует.

**При двух и более ДЦ возможны:**

- **Weighted DNS:** выдача нескольких A-записей с разными весами у провайдера (например, 80% — ДЦ1, 20% — ДЦ2) для поэтапного перевода трафика.
- **Traffic steering у CDN:** правила по гео, AS или заголовкам направляют запросы на тот или иной origin (ДЦ).
- **Переключение при сбое:** снижение веса или исключение failing ДЦ из DNS/CDN, возврат после проверки здоровья.
...
2. Airbnb Statistics 2026: [https://www.demandsage.com/airbnb-statistics](https://www.demandsage.com/airbnb-statistics)
3. Hotelagio, Airbnb statistics: [https://hotelagio.com/airbnb-statistics](https://hotelagio.com/airbnb-statistics)
4. Business of Apps, Airbnb: [https://www.businessofapps.com/data/airbnb-statistics/](https://www.businessofapps.com/data/airbnb-statistics/)
5. Airbnb Q3 2024 Shareholder Letter: [https://investors.airbnb.com](https://investors.airbnb.com)
...
7. Airbnb Newsroom: [https://news.airbnb.com/about-us](https://news.airbnb.com/about-us)
8. HTTP Archive / Web Almanac: [https://almanac.httparchive.org](https://almanac.httparchive.org)
