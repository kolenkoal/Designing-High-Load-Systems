...
После прохождения глобального уровня `DNS → CDN/WAF → ДЦ` входящий трафик в датацентре попадает на **NGINX Ingress Controller**.

NGINX Ingress Controller выполняет:

* терминацию TLS для соединений от CDN/WAF до датацентра;
* маршрутизацию по hostname (`www.airbnb.com`, `api.airbnb.com`);
* health-check backend-сервисов;
* балансировку запросов между репликами сервисов.

Для MVP используется схема active-active с двумя экземплярами NGINX Ingress Controller. При отказе одного экземпляра второй продолжает принимать весь входящий трафик.

Сервисы считаются stateless, поэтому sticky sessions не используются. Состояние пользователя хранится в базе данных и внешних хранилищах.

```mermaid
flowchart LR
U[Пользователь] --> DNS[DNS]
DNS --> EDGE[CDN/WAF]
EDGE --> IN[NGINX Ingress Controller]
...
Межсервисное взаимодействие реализовано средствами Kubernetes Service Discovery.

Каждый сервис публикуется в Kubernetes как объект Service, после чего становится доступен внутри кластера по DNS-имени через **CoreDNS**.
Актуальный список pod-ов сервиса хранится в EndpointSlice.
Распределение трафика между pod-ами выполняет kube-proxy в режиме IPVS.

То есть схема работы такая:

1. сервис A обращается к сервису B по DNS-имени;
2. CoreDNS возвращает виртуальный IP сервиса B;
3. kube-proxy/IPVS распределяет запрос на один из pod-ов сервиса B.

Минимальные механизмы надёжности для межсервисных вызовов:
...
```mermaid
flowchart LR
A[Service A] --> DNSK[CoreDNS]
DNSK --> VIP["Service B (ClusterIP)"]
VIP --> B1["Pod 1"]
VIP --> B2["Pod 2"]
VIP --> B3["Pod 3"]
```

### Распределение запросов внутри кластера

| Тип взаимодействия       | Механизм                 |
| ------------------------ | ------------------------ |
| Внешний запрос → сервис  | NGINX Ingress Controller |
| Сервис → сервис          | Kubernetes Service       |
| Разрешение имени сервиса | CoreDNS                  |
| Хранение списка pod-ов   | EndpointSlice            |
| Балансировка по pod-ам   | kube-proxy (IPVS)        |

## 4.3 Схема отказоустойчивости

Так как используется один датацентр, полный отказ датацентра в рамках MVP не покрывается.
При этом внутри датацентра обеспечивается отказоустойчивость на трёх уровнях.

### Уровень приложения

* Для каждого критичного сервиса запускается 2–3 реплики.
* Неработающие реплики исключаются из балансировки по результатам health-check.
* Реплики размещаются на разных узлах кластера, чтобы отказ одного узла не приводил к остановке сервиса.

### Уровень входа

* NGINX Ingress Controller запускается в двух экземплярах.
* При отказе одного ingress весь трафик принимает второй.

### Деградация вместо полного отказа

При проблемах в некритичных подсистемах могут временно отключаться второстепенные функции, но ключевой пользовательский сценарий сохраняется:

поиск → карточка объявления → проверка дат → бронирование
...
Пиковый RPS приложения:

**RPS_peak = 20 895**

Тогда количество TLS-рукопожатий в секунду:

$R_{TLS} = \frac{RPS_{peak}}{K}$

При `**K = 1**`:

$R_{TLS} = \frac{20895}{1} = 20895 \text{ TLS-рукопожатий/с}$

### Оценка CPU для TLS-терминации

По отчёту NGINX SSL Performance принимаем консервативную оценку:

1 vCPU ≈ 350 полных TLS-рукопожатий/с

Тогда требуемое количество ядер:

$CPU = \frac{20895}{350} \approx 59.7$
