...
|--------|--------------|------------------|------------|
| **Чтение, кэшируемое** | Поиск, выдача объявлений, превью | Поиск (200 млн/день), страницы объявлений (175 млн/день), проверка доступности дат (175 млн/день) | Низкая задержка, допустима итоговая согласованность, раздача через CDN |
...
| **Запись, критичная** | Транзакции и мутации | Бронирование (1.34 млн/день), новые объявления (50 тыс/день), отзывы (500 тыс/день) | Согласованность и надёжность, один источник истины (основной ДЦ) |
...
- Центральное положение между основными рынками (Европа, частично Ближний Восток и Африка), приемлемое время задержки туда-обратно для значимой доли DAU.
- Развитая инфраструктура, надёжные каналы связи и соответствие регуляториям (GDPR и др.).
- Один ДЦ упрощает согласованность данных (бронирования, платежи), снижает стоимость и сложность на этапе MVP.

## 3.3 Схема глобальной балансировки до датацентров

При одном ДЦ схема плоская: весь трафик мимо CDN направляется в этот ДЦ.
<img width="963" height="533" alt="image" src="https://github.com/user-attachments/assets/12bbaa97-0bb3-4c1c-af7e-2562ccad3eb8" />

По группам нагрузки:

- **Кэшируемое чтение (поиск, объявления, доступность):** запрос может обслуживаться CDN; при промахе кэша — запрос в основной ДЦ.
- **Персональное чтение и запись:** все запросы идут в основной ДЦ (единая точка согласованности).

## 3.4 Функциональное разбиение по доменам

Для связи балансировки с продуктом выделяются домены:

| Домен | Назначение | Отношение к ДЦ |
|-------|------------|----------------|
| **Поиск и каталог** | Поиск, каталог, карточки объявлений, превью | Кэш на CDN, источник в ДЦ |
| **Доступность** | Календарь, проверка дат | API в ДЦ, при необходимости кэш по объявлению |
| **Бронирования** | Бронирование, отмена, история | Только ДЦ (транзакции) |
| **Платежи** | Платёжный провайдер, статусы | Только ДЦ |
| **Идентификация и профиль** | Авторизация, профиль, настройки | Только ДЦ (сессии, персональные данные) |
| **Отзывы** | Отзывы и рейтинги | Запись в ДЦ, чтение можно кэшировать |
...
|---------------------|-------------------------|
| **Конверсия поиск → бронирование** | Задержка ответа поиска и страницы объявления влияет на отказы. Снижение: отдача поиска и медиа через CDN по географии пользователя; в ДЦ идут только промахи кэша и персональные запросы. |
| **Время до первого байта** | Европейский ДЦ даёт приемлемую задержку туда-обратно (50–150 мс) для Европы и части СНГ/Ближнего Востока; для Америки и Азии задержка выше — компенсируется кэшем поиска и медиа в CDN. |
| **Надёжность бронирований и платежей** | Один ДЦ — одна зона согласованности: нет конфликтов репликации, проще соблюдать транзакционную целостность и корректность финансовых операций. |
| **MAU/DAU и удержание** | Стабильность и предсказуемая задержка важнее экстремально низкой задержки для туристического сценария; один правильно выбранный ДЦ достаточен для MVP. |

Итог: размещение основного ДЦ в Западной Европе при разгрузке через CDN даёт приемлемые задержки для большей части пользователей и сохраняет простоту и надёжность для операций, критичных к согласованности данных.
...
| Тип запроса | Запросов/день | RPS (пик) | Обработка в ДЦ | Комментарий |
|-------------|---------------|-----------|----------------|-------------|
| Поиск | 200 млн | 6945 | Часть (промах кэша) | Основной объём отдаёт CDN и поисковый кэш |
| Страницы объявлений | 175 млн | 6075 | Часть (промах кэша) | Метаданные и API — ДЦ, медиа — CDN |
| Проверка доступности дат | 175 млн | 6075 | Полностью или через кэш | Критично к актуальности, при кэше — короткое время жизни записи |
| Бронирование | 1.34 млн | 47 | 100% | Только основной ДЦ |
| Новые объявления | 50 тыс | 2 | 100% | Только ДЦ |
| Отзывы | 500 тыс | 17 | 100% | Только ДЦ |
| Авторизация / профиль | 50 млн | 1734 | 100% | Только ДЦ (сессии) |

Оценка RPS в ДЦ при 30% промахов кэша для поиска и страниц объявлений: (6945 + 6075) × 0.3 + 6075 + 47 + 2 + 17 + 1734 ≈ 3906 + 6075 + 1800 ≈ **~11 800 RPS пик** в один ДЦ (остальное забирают CDN и кэш).

## 3.7 Схема DNS балансировки (при наличии)

При одном ДЦ балансировка между несколькими ДЦ не используется: одна запись указывает на основной ДЦ (или на балансировщик перед ним). Этого достаточно для текущей конфигурации.

## 3.8 Схема Anycast балансировки (при наличии)

**Anycast** — один IP-адрес объявляется из нескольких точек присутствия; протокол BGP направляет клиента в «ближайший» по маршруту узел. При одном ДЦ Anycast не даёт выигрыша (одна точка объявления IP), поэтому для текущей конфигурации не применяется.

## 3.9 Механизм регулировки трафика между ДЦ (при наличии)

При одном ДЦ регулировка трафика между датацентрами не требуется.
