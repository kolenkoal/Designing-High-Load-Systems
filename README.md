...
Основной продуктовый риск “далёкого” датацентра — рост задержек на интерактивные операции (поиск, карточка, проверка дат, бронирование). Увеличение задержки на десятки–сотни миллисекунд ухудшает поведение пользователей и конверсию [[1]](https://www.thinkwithgoogle.com/_qs/documents/9757/Milliseconds_Make_Millions_report_hQYAbZJ.pdf).
...
Вывод: один ДЦ в Западной Европе + CDN для контента даёт хороший баланс скорости простоты для MVP.
...
1. Milliseconds make Millions – A study on how improvements in mobile site speed positively affect a brand’s bottom line: [https://www.thinkwithgoogle.com](https://www.thinkwithgoogle.com/_qs/documents/9757/Milliseconds_Make_Millions_report_hQYAbZJ.pdf)

---

# 4. Локальная балансировка

## 4.1 Схемы балансировки для входящих запросов

Глобально входящий трафик приходит через: DNS → CDN/WAF → основной ДЦ.

Внутри ДЦ для входящих запросов используем L7-балансировку на входе, ingress, а именно Ingress Controller или reverse-proxy на входе:

- Терминация TLS (если между CDN/WAF и ДЦ используется HTTPS).
- Маршрутизация по hostname (`www.airbnb.com`, `api.airbnb.com`) и путям.
- Health-check бэкендов и исключение недоступных инстансов.
- Балансировка по репликам сервиса round-robin.

Сессии не “прилипают” к одному серверу (без sticky sessions): сервисы считаются stateless, а состояние хранится в БД/хранилищах.
...
Межсервисные запросы балансируются через внутренний DNS:

* каждый сервис регистрирует несколько инстансов;
* клиент обращается по имени сервиса (например, `booking-service`);
* Service discovery возвращает адрес сервиса (VIP) или набор адресов инстансов. Дальше запрос распределяется между репликами (round-robin / ipvs и т.п.).

Минимальные практики надёжности для межсервисных вызовов:

* таймауты на каждый запрос,
* ретраи только для идемпотентных операций,
* circuit breaker, который можно поставить на уровне библиотеки клиента или входного прокси.
...
Так как используется один ДЦ, полный отказ ДЦ не покрывается для упрощения MVP. При этом обеспечиваем устойчивость к типовым отказам внутри ДЦ:

### Уровень приложения

* 2–3 реплики каждого критичного сервиса.
* Health-check и автоматическое исключение неработающих реплик из балансировки.
* Размещение реплик на разных узлах (или зонах внутри ДЦ), чтобы отказ одного узла не приводил к недоступности сервиса.

### Уровень входа

* Входной L7-балансировщик запускается минимум в 2 экземплярах.
* При падении одного экземпляра трафик принимает второй.

### Уровень БД

* Основная БД + одна или несколько реплик (репликация в режиме streaming).
* Автоматическое переключение на реплику при отказе основной БД (failover).
* Регулярные бэкапы + возможность восстановления до точки во времени, использование алгоритма PITR [[1]](https://www.postgresql.org/docs/current/continuous-archiving.html)

### Деградация вместо падения

Некритичные функции (например, часть витринных блоков/рекомендаций) могут временно отключаться при проблемах, при сохранении ключевого сценария: поиск → карточка → проверка дат → бронирование.

## 4.4 Расчёт нагрузки на терминацию SSL (TLS)

### Где происходит терминация TLS

* TLS от пользователя терминируется на CDN/WAF.
* Если канал CDN/WAF → ДЦ защищён (HTTPS), то TLS повторно терминируется на входном L7-балансировщике в ДЦ.

Нагрузка на терминацию TLS определяется количеством новых соединений (TLS-рукопожатий/с), а не RPS напрямую, так как одно соединение обслуживает много запросов (keep-alive/HTTP2).
Считаем, что CDN переиспользует соединения к ДЦ (keep-alive/HTTP2).

### Исходные данные

Пиковый RPS приложения: **RPS_peak = 20 895** (раздел 2.2.3).

Пусть в среднем одно TLS-соединение CDN/WAF → ДЦ обслуживает `K` запросов, тогда число TLS-рукопожатий в секунду:

$R_{TLS} = \frac{RPS_{peak}}{K}$

Рассмотрим два сценария:

**Сценарий A (реалистичный):** K = 50  
$R_{TLS} \approx \frac{20895}{50} \approx 418 \text{ TLS-рукопожатий/с}$

**Сценарий B (консервативный):** K = 10  
$R_{TLS} \approx \frac{20895}{10} \approx 2090 \text{ TLS-рукопожатий/с}$

### Оценка CPU для TLS-терминации

Консервативная оценка: порядка **~350 полных TLS-рукопожатий/с на 1 vCPU** (оценка зависит от TLS/шифров и железа; берём с запасом) [[2]](https://cdn.studio.f5.com/files/k6fem79d/production/2805bffce067ef760a8fa8939d7dd8b443a5e5f6.pdf).

Тогда требуемые ядра:

* Сценарий A: 418 / 350 ≈ 1.2 → 2 vCPU
* Сценарий B: 2090 / 350 ≈ 6.0 → 6 vCPU

С учётом запаса и схемы active-active (2 экземпляра входного L7), можно заложить:

* 2 экземпляра L7-балансировщика по 4 vCPU каждый (в сумме 8 vCPU).

## Список источников
1. PITR в PostgreSQL – [https://www.postgresql.org/docs/current/continuous-archiving.html](https://www.postgresql.org/docs/current/continuous-archiving.html)
2. NGINX SSL Performance Report – [https://cdn.studio.f5.com/files/k6fem79d/production/2805bffce067ef760a8fa8939d7dd8b443a5e5f6.pdf](https://cdn.studio.f5.com/files/k6fem79d/production/2805bffce067ef760a8fa8939d7dd8b443a5e5f6.pdf)