...
Обоснование размера `app-pool`: суммарный CPU limit всех контейнеров (таблица 11.2.2) — 111 vCPU. При профиле 16 vCPU на ноду: ⌈111/16⌉ = 7 рабочих нод + 1 резервная = **8 нод**.

### 11.2.2 Контейнеры приложения (`app-pool`)

Пиковый RPS из раздела 2.2.3. Производительность FastAPI (async I/O) принята ≈ 500 RPS/vCPU, API Gateway — ≈ 1 000 RPS/vCPU (лёгкая работа: JWT-валидация + проксирование).

| Сервис | Реплики | CPU request | CPU limit | RAM request | RAM limit | Обоснование реплик |
|--------|---------|------------|----------|------------|----------|-------------------|
| `api-gateway` | **11** | 1000m | 2000m | 256 Mi | 512 Mi | 20 895 пик RPS ÷ 1 000 = 21 vCPU → 11 реплик × 2 |
...
**Суммарно (лимиты):** 11×2 + 14×1 + 13×2 + 4×1 + 3×1 + 3×1 + 2×1 + 5×2 + 3×1 + 3×2 = **111 vCPU**, ~27 ГБ RAM limit.
