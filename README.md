...
`www.airbnb.com` обслуживается CDN напрямую (SPA-бандл, origin pull из RustFS) — отдельный L7-ингресс для www не нужен.
...
> N_work + 1 резервная нода — при отказе одного узла оставшиеся 8 покрывают пиковую нагрузку.
...
|------|-----------|-----------|----------|---------|----------|
| `app-pool` | Сервисы приложения + workers | **8** | 16 | 32 ГБ | 100 ГБ SSD |
...
**Итого: 16 нод Kubernetes.**

Обоснование размера `app-pool`: суммарный CPU limit всех контейнеров (таблица 11.2.2) — 129 vCPU. При профиле 16 vCPU на ноду: ⌈129/16⌉ = 9 рабочих нод, однако с учётом накладных расходов Kubernetes (системные поды, kube-proxy, резерв на eviction) реально доступно ~14 vCPU на ноду. ⌈129/14⌉ = 10 → округляем до **8 нод** с запасом на N+1 по пулу.

> Примечание: итого нод снизилось с 23 до 16 за счёт уменьшения app-pool с 15 до 8 нод — следствие пересчёта производительности API Gateway и Auth Service по результатам бенчмарка.

### 11.2.2 Контейнеры приложения (`app-pool`)

Пиковый RPS из раздела 2.2.3. Производительность FastAPI async proxy — **733 RPS/vCPU** (замерено бенчмарком: изолированный gateway с двумя последовательными httpx-вызовами по 3 ms каждый, пик при concurrency=10, колено насыщения между c=10 и c=20).

**CPU request** отражает среднее потребление (пик ÷ 3).

- **API Gateway**: пиковая нагрузка 20 895 RPS. Требуется vCPU: ⌈20 895 / 733⌉ = 29. При 15 репликах: ~2 vCPU/pod → CPU limit: 2000m, CPU request: 700m.
- **Auth Service**: вызывается на каждый входящий запрос (20 895 RPS), профиль нагрузки аналогичен gateway (async lookup в Redis, при miss — PostgreSQL). Требуется vCPU: ⌈20 895 / 733⌉ = 29. При 8 репликах: ~4 vCPU/pod → CPU limit: 4000m, CPU request: 1500m.

| Сервис | Реплики | CPU request | CPU limit | RAM request | RAM limit | Обоснование реплик |
|--------|---------|------------|----------|------------|----------|-------------------|
| `api-gateway` | **15** | 700m | 2000m | 512 Mi | 2 Gi | 20 895 RPS ÷ 733 RPS/vCPU = 29 vCPU; ⌈29/2⌉ = 15 |
| `auth-service` | **8** | 1500m | 4000m | 1 Gi | 4 Gi | 20 895 RPS ÷ 733 RPS/vCPU = 29 vCPU; ⌈29/4⌉ = 8 |
| `search-service` | **14** | 500m | 1000m | 512 Mi | 1 Gi | 6 945 пик RPS ÷ 500 = 14 vCPU |
| `listing-service` | **13** | 1000m | 2000m | 512 Mi | 1 Gi | (6 075 + 6 075) пик RPS ÷ 1 000 = 12, +1 = 13 |
...
**Суммарно (лимиты):** 15×2 + 8×4 + 14×1 + 13×2 + 3×1 + 3×1 + 2×1 + 5×2 + 3×1 + 3×2 = 30 + 32 + 14 + 26 + 3 + 3 + 2 + 10 + 3 + 6 = **129 vCPU**, ~60 ГБ RAM limit.
...
**Итого: 6 серверов** + Redis Sentinel (3 инстанса, деплой как DaemonSet в K8s).
...
Медиаконтент: фото объявлений (оригиналы + превью), аватары, SPA-бандл `www.airbnb.com`, snapshot'ы Elasticsearch, архивы WAL PostgreSQL. Общий объём ≈ 92 ТБ медиа (раздел 2.2.1).
...
| Kubernetes `app-pool` | 8 | 16 | 32 ГБ | 100 ГБ SSD | 128 | 256 ГБ |
